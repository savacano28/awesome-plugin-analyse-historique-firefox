...
class FiryKafkaResourceIT {
    ...
    private FiryKafkaService firyKafkaService;
    ...
    void setup() {
        ...
        FiryKafkaResource kafkaResource = new FiryKafkaResource(firyKafkaService);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/firy-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/firy-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
